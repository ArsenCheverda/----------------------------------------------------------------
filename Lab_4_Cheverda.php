...
// Інтерфейс
// Цей інтерфейс дано і ми не можемо його змінювати.
...
class EmailNotification implements Notification
{
    ...
    {
        mail($this->adminEmail, $title, $message); // Логіка відправки
        echo "Sent email with title '$title' to '{$this->adminEmail}' that says '$message'.\n";
    }
}

// Сторонні сервіси з несумісними інтерфейсами, які ми хочемо інтегрувати.
...
// Класи-адаптери які "адаптують" несумісні інтерфейси SlackApi та SmsService до нашого цільового інтерфейсу Notification.
...
class SmsNotificationAdapter implements Notification
{
    ...
    public function send(string $title, string $message): void
    {
        // Форматуємо повідомлення для SMS
...
// Створюємо екземпляр існуючої системи
$emailNotification = new EmailNotification('admin@example.com');

// Створюємо екземпляри нових сервісів
$slackApi = new SlackApi();
$smsService = new SmsService();

// Створюємо екземпляри Адаптерів, огортаючи нові сервіси та надаючи необхідну конфігурацію
...
// Клієнтський код працює з усіма об'єктами через єдиний інтерфейс Notification
...
// SMS
sendNotification($smsAdapter, 'Critial', 'Server is down! Check immediately!');
